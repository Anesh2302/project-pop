...
def build_html(md, repo):
    ...
    for line in md.split("\n"):
        if line.startswith("# "):
            ...
            if in_list: body_lines.append("</ul>"); in_list = False
            body_lines.append(f"<h2 style=\'color:#0d1117;font-size:15px;font-weight:600;margin:20px 0 6px;\'>{line[3:]}</h2>")
        elif line.strip() == "---":
            if in_list: body_lines.append("</ul>"); in_list = False
            body_lines.append("<hr style=\'border:none;border-top:1px solid #e5e7eb;margin:20px 0\'>")
        elif line.startswith("- ") or line.startswith("* "):
            if not in_list: body_lines.append("<ul style=\'margin:6px 0;padding-left:20px\'>"); in_list = True
    ...
    if in_list: body_lines.append("</ul>")
    body_html = "\n".join(body_lines)
    return f"""<!DOCTYPE html><html><body style="margin:0;padding:0;background:#f3f4f6;font-family:-apple-system,sans-serif">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f3f4f6;padding:32px 16px">
    <tr><td align="center"><table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:12px;border:1px solid #e5e7eb;max-width:600px">
      <tr><td style="background:#0d1117;padding:20px 28px;color:#fff">
